            ...
            >
              ...
              <div class="carousel-indicator">
                ...
                <div class="post-actions">
                  ...
                  <button @click="openCommentModal(post)"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-message-circle-more-icon lucide-message-circle-more"><path d="M2.992 16.342a2 2 0 0 1 .094 1.167l-1.065 3.29a1 1 0 0 0 1.236 1.168l3.413-.998a2 2 0 0 1 1.099.092 10 10 0 1 0-4.777-4.719"/><path d="M8 12h.01"/><path d="M12 12h.01"/><path d="M16 12h.01"/></svg> <span>Comment</span></button>
                  <button @click="openShareModal(post)"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-share2-icon lucide-share-2"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" x2="15.42" y1="13.51" y2="17.49"/><line x1="15.41" x2="8.59" y1="6.51" y2="10.49"/></svg> <span>Share</span></button>
                  <button @click="toggleSavePost(post)" :class="{ active: isSaved(post) }">
                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" 
                      viewBox="0 0 24 24" :fill="isSaved(post) ? '#FFFF00' : 'none'" :stroke="isSaved(post)? '#FFFF00': 'currentColor'" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-bookmark-icon lucide-bookmark"><path d="M17 3a2 2 0 0 1 2 2v15a1 1 0 0 1-1.496.868l-4.512-2.578a2 2 0 0 0-1.984 0l-4.512 2.578A1 1 0 0 1 5 20V5a2 2 0 0 1 2-2z"/>
                    </svg>
                    <span>{{ isSaved(post) ? 'Saved' : 'Save' }}</span></button>
                </div>
              </div>
                   ...
                   <template v-else>
                      ...
                      <div class="post-actions">
                          <button @click="openCommentModal(post.post)">
                            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-arrow-big-up-icon lucide-arrow-big-up"><path d="M9 13a1 1 0 0 0-1-1H5.061a1 1 0 0 1-.75-1.811l6.836-6.835a1.207 1.207 0 0 1 1.707 0l6.835 6.835a1 1 0 0 1-.75 1.811H16a1 1 0 0 0-1 1v6a1 1 0 0 1-1 1h-4a1 1 0 0 1-1-1z"/></svg> Open Origin Post
                          </button>
                      </div>
                   </template>
                </template>
...
import EditShareModal from '../feed/EditShareModal.vue';
import CommentModal from '../feed/CommentModal.vue';
import ShareModal from '../feed/ShareModal.vue';
import EditPostModal from '../feed/EditPostModal.vue';
...
export default {
  ...
  data() {
    return {
      ...
      confirmMessage: '',
      pendingAction: null,   // ← gom tất cả action vào đây
      pendingData: null, 
...
async fetchFriends(page = 1) {
  ...
  return post.author._id === savedUserId;
    ...
    openImageViewer(imageUrl) {
      this.closeMenus();
      if (!imageUrl) {
        this.showNotify("error", "Error", "No image to preview!");
        return;
      }
    ...
    deleteShare(shareId) {
      ...
      this.pendingAction  = 'deleteShare'; // ← thêm dòng này
      ...
      try {
        ...
        if (res.ok) {
          if (isSaved) {
           ...
           this.showNotify("success", "Success!", data.msg || 'Failed to save/unsave item');
        }
      } catch (err) {
        console.error("Cannot save/unsave item:", err);
        this.showNotify("success", "Success", "Unable to save/unsave item");
      }
    },
...
async handleFriendAction() {
  ...
  if (this.friendStatus === 'sent') {
    this.confirmMessage = 'Do you want to cancel this friend request?';
    this.pendingAction  = 'cancelFriend';
    this.confirmVisible = true;
    return;
  }

  if (this.friendStatus === 'friends') {
    this.confirmMessage = 'Do you want to unfriend this user?';
    this.pendingAction  = 'unfriend';
    this.confirmVisible = true;
    return;
  }
  ...
  try {
    ...
    async handleBlock() {
      ...
      this.confirmMessage = `Block ${this.profileUser?.firstname} ${this.profileUser?.lastname}?`;
      this.confirmVisible = true;
    },
    ...
    async handleUnblock() {
      ...
      this.confirmMessage = `Unblock ${this.profileUser?.firstname} ${this.profileUser?.lastname}?`;
      this.confirmVisible = true;
    },
...
async fetchUserMedia(page = 1) {
  try {
    ...
    try {
      ...
      this.showNotify("error", "Error", "Cannot open this post");
    }
    },
...
async handleConfirmedAction() {
  ...
  if (action === 'deletePost') {
    try {
      const res = await fetch(`http://localhost:3000/posts/${this.postToDeleteId}`, { method: 'DELETE' });
      if (res.ok) {
        this.userPosts = this.userPosts.filter(p => p._id !== this.postToDeleteId);
        await this.fetchUserPosts();
        this.openMenuId = null;
        this.showNotify("success", "Success", "Post deleted.");
      } else { this.showNotify("error", "Error", "Cannot delete post!"); }
    } catch { this.showNotify("error", "Error", "Network error."); }
  }

  // ── Xóa share ─────────────────────────────────────────────
  else if (action === 'deleteShare') {
    try {
      const res = await fetch(`http://localhost:3000/shares/${this.postToDeleteId}`, { method: 'DELETE' });
      if (res.ok) {
        await this.fetchUserPosts();
        this.showNotify("success", "Success", "Share deleted.");
      } else { this.showNotify("error", "Error", "Cannot delete share!"); }
    } catch { this.showNotify("error", "Error", "Network error."); }
  }
  ...
  // ── Cancel friend request ──────────────────────────────────
    ...
    try {
      await fetch("http://localhost:3000/users/friend-request/cancel", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ fromUserId: viewerId, toUserId: this.profileUser._id })
      });
      await this.fetchUserProfile();
      await this.fetchFriends(this.pageFriends);
      window.dispatchEvent(new Event('friend-status-changed'));
    } catch (err) { console.error(err); }
    finally { this.loadingFriend = false; }
  }
  ...
  // ── Unfriend ───────────────────────────────────────────────
    ...
    try {
      await fetch("http://localhost:3000/users/unfriend", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ userId: viewerId, friendId: this.profileUser._id })
      });
      await this.fetchUserProfile();
      await this.fetchFriends(this.pageFriends);
      window.dispatchEvent(new Event('friend-status-changed'));
  ...
  // ── Unblock ────────────────────────────────────────────────
    ...
    try {
      ...
      if (res.ok) {
        ...
        const d = await res.json().catch(() => ({}));
        this.showNotify("error", "Error", d.msg || "Unable to unblock.");
      }
    } catch { this.showNotify("error", "Network error", "Unable to connect to server."); }
    finally { this.loadingBlock = false; }
  }
