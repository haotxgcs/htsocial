  ...
  try {
    ...
    async hideThisPost(postId) {
      ...
      try {
        ...
        const data = await res.json();
        this.showNotify("success", "Success", data.msg);
        this.posts = this.posts.filter(p => p._id !== postId);
      } catch (err) {
        console.error("Error hiding post:", err);
        this.showNotify("error", "Error", "Failed to hide post");
      }
    },
    ...
    async hideThisShare(shareId) {
      ...
      try {
        ...
        this.showNotify("success", "Success", data.msg || "Hide share successfully!");
        await this.fetchPosts();
      } catch (err) {
        console.error("Error to hide share:", err);
        this.showNotify("error", "Error", err.message || "Unable to hide share");
      }
    },
    ...
    async handleConfirmedAction() {
       ...
       try {
          ...
          if(this.deleteType === 'share') {
             ...
             } else this.showNotify("error", "Error", "Error deleting share");
          ...
          // --- TRƯỜNG HỢP 2: XÓA BÀI GỐC (SỬA Ở ĐÂY) ---
             ...
             } else this.showNotify("error", "Error", "Error deleting post");
          }
       } catch(e) { 
           console.error(e);
           alert("Server error"); 
...
.post-actions button.active img { filter:none; opacity: 1; color: blue;}
.post-action img:hover { background: red;}
