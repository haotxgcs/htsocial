...
handleFiles(e) {
  ...
  if (this.previews.length + newFiles.length > 5) {
    this.showNotify("error", "Error", "You can upload a maximum of 5 images.");
    e.target.value = "";
    return;
  }
  ...
  e.target.value = ""; // cho phép upload lại cùng file
    ...
    async submit() {
      ...
      if (!token) return this.showNotify("error", "Error", "Login required");
      ...
      if (!res.ok) return this.showNotify("error", "Error", "Update failed");
