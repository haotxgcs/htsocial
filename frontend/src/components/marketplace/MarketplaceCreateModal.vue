<template>
  <div
    v-if="isVisible"
    class="create-item-modal-overlay"
    @click.self="!loading && requestClose()"
  >
    <div class="create-item-modal-content">

      <!-- HEADER -->
      <div class="create-item-modal-header">
  ...
  methods: {
    handleFiles(e) {
      ...
      if (files.length < 1 || files.length > 5) {
        this.showNotify("error", "Error", "Please upload 1-5 images");
        return;
      }
      ...
      if (!valid) {
        this.showNotify("error", "Error", "Images must be JPG/PNG and ≤ 5MB");
        return;
      }
...
async submit() {
  ...
  if (!token) return this.showNotify("error", "Error", "Please login first");
  ...
  if (!res.ok) {
    ...
    this.showNotify("error", "Error", data.msg || "Create item failed");
    return;
  }
...
.create-item-modal-overlay { 
  ...
  align-items: center; z-index: 1000; animation: fadeIn 0.2s ease-out; 
}
...
.create-item-modal-header h3 { 
  margin: 0; font-size: 20px; font-weight: 700; 
  color: var(--text-main); text-align: center; flex: 1; 
}
.close-btn { 
  position: absolute; right: 16px; background: var(--bg-input); 
  border: none; border-radius: 50%; width: 36px; height: 36px; 
  display: flex; align-items: center; justify-content: center; cursor: pointer; 
  font-size: 20px; color: var(--text-sub); transition: background-color 0.2s; 
}
.close-btn:hover { background: var(--hover-bg); }
.creator-info { display: flex; align-items: center; padding: 16px 24px; gap: 12px; }
.creator-avatar { width: 40px; height: 40px; border-radius: 50%; object-fit: cover; }
.creator-details { flex: 1; }
.creator-details strong { display: block; font-size: 15px; font-weight: 600; color: var(--text-main); margin-bottom: 4px; }

.post-content-area { padding: 0 24px; position: relative; }
.recipe-field { margin-bottom: 20px; }
.field-label { display: block; font-size: 16px; font-weight: 600; color: var(--text-main); margin-bottom: 10px; }
.recipe-input, .category-select { 
  width: 100%; padding: 12px; font-size: 15px; font-family: inherit; line-height: 1.2; 
  color: var(--text-main); background: var(--bg-input); border: 1px solid var(--border-color); 
  border-radius: 8px; box-sizing: border-box; transition: border-color 0.2s, background-color 0.2s; 
}
...
.recipe-input:focus, .category-select:focus { 
  border-color: #FF642F; background: var(--bg-card); outline: none;  
}
.recipe-textarea { 
  width: 100%; min-height: 100px; padding: 12px; font-size: 15px; font-family: inherit; line-height: 1.4; 
  color: var(--text-main); background: var(--bg-input); border: 1px solid var(--border-color); 
  border-radius: 8px; box-sizing: border-box; resize: vertical; transition: border-color 0.2s, background-color 0.2s; 
}
.recipe-textarea:focus { border-color: #FF642F; color:var(--text-main); background:var(--bg-card); outline: none;  }
...
@media (max-width: 768px) { .create-item-modal-content { width: 95%; margin: 20px; max-height: calc(100vh - 40px); } .post-textarea { font-size: 18px; } .add-options { flex-direction: column; align-items: stretch; } .add-option { justify-content: flex-start; } }
.create-item-modal-content::-webkit-scrollbar { width: 6px; }
.create-item-modal-content::-webkit-scrollbar-track { background: #f8f9fa; border-radius: 3px; }
.create-item-modal-content::-webkit-scrollbar-thumb { background: #bcc0c4; border-radius: 3px; }
.create-item-modal-content::-webkit-scrollbar-thumb:hover { background: #8a8d91; }
.sub-text {
  font-size: 13px;
  color: var(--text-sub);
}
