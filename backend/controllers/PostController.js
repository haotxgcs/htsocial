...
exports.getSavedPostIds = async (req, res) => {
  try {
    ...
    // Trả về danh sách ID của saved posts
...
exports.getSavedPosts = async (req, res) => {
  try {
    ...
    const user = await User.findById(userId).populate({
      ...
      populate: {
        path: 'author',
        select: 'firstname lastname avatar username friends'
      },
    ...
    const visibleSavedPosts = user.savedPosts.filter(post => {
      if (!post || !post.author) return false; // Post có thể đã bị xóa
    ...
    const savedPostsWithCounts = visibleSavedPosts.map(post => ({
      ...post.toObject(),
    ...
    console.error('Get saved posts for display error:', error);
    res.status(500).json({ msg: 'Server error' });
  }
};
